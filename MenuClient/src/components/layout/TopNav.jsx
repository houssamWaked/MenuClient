    ...
    <header className={`top-nav ${isHome ? 'top-nav-home' : ''}`}>
      ...
      <nav className="top-nav-links">
        {navigationItems.map((entry) => {
          const matchPaths = Array.isArray(entry.matchPaths) && entry.matchPaths.length ? entry.matchPaths : [entry.href];
          const isActive = matchPaths.some((matchPath) => normalizePath(matchPath) === pathname);
      ...
      <div className="top-nav-actions">
        <button
          ...
          aria-label={cartLabel || 'Open cart'}
        >
        ...
        <button type="button" className="book-btn" onClick={onBook}>
          <span className="book-btn-label">{bookLabel}</span>
